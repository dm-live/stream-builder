...
def build_random_playlist(scene_paths):
    print("Building randomized playlist (real durations)…")
    ...
    for p in scene_paths:
        d = get_duration(p)
        durations[p] = d
        print(f"  Duration: {p} = {round(d,2)}s")

    playlist = []
    total_duration = 0

    last_clip = None  # avoid immediate repeats

    while total_duration < TARGET_DURATION:
        shuffled = scene_paths.copy()
        ...
        for clip in shuffled:
            ...
            total_duration += durations[clip]
            last_clip = clip

            if total_duration >= TARGET_DURATION:
                break

    print(f"Final playlist duration ≈ {int(total_duration)}s")
    return playlist
